...
for s in switches:
    temp_hosts = ()
    for h in all_hosts:
        if hosts_dic[h]['id'] == switches_dic[s]['id']: #checking the OF id
            temp_hosts += (h,)
...
#### either of the connected states ####
    if len(txt)==1:
        ...
        f.writelines("};")
    f.writelines('\n')
##### Checking the mission requirments file ####
...
for x in range(len(miss['missionRequirements'])):
    ...
    for s in switches_dic:
        ...
        if src in switches_dic[s]['hosts']:
            switch = hosts_dic[dst]['switch']
            f.writelines('        switch = ' +str(s)+ ' & src = ' + src +' & dst = ' + dst +':' + switch + ';\n')
...
f.close()
##### Getting into Mission Requirments #####
